...
from src.repository.contacts import get_contacts, get_contact, create_contact, update_contact, delete_contact, \
    search_contacts, get_upcoming_birthdays, adjust_for_weekend, date_to_string
...
class TestAsyncContact(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_contacts(self):
        limit = 10
        offset = 0
        contacts = [Contact(id=1, first_name='contact_1', email='email_1', user=self.user),
    ...
    async def test_create_contact(self):
        body = ContactSchema(first_name='John', last_name='Snow', email='email', phone='123456789',
                             birthday=date.today(), data_add='data')
    ...
    async def test_update_contact(self):
        ...
        mocked_contact.scalar_one_or_none.return_value = Contact(id=1, first_name='John', email='email', user=self.user)
        self.session.execute.return_value = mocked_contact
    ...
    async def test_search_contacts_not_found(self):
        ...
        self.session.execute.return_value = mocked_contacts
        result = await search_contacts(self.session, self.user, first_name='John', last_name='Doe', email='email')
        self.assertEqual(result, [])

    async def test_search_contacts_no_params(self):
        contacts = [
            Contact(id=1, first_name='John', last_name='Doe', email='email_1', user=self.user),
            Contact(id=2, first_name='Jane', last_name='Smith', email='email_2', user=self.user)
        ]
        mocked_contacts = MagicMock()
        mocked_contacts.scalars.return_value.all.return_value = contacts
        self.session.execute.return_value = mocked_contacts
    ...
    async def test_search_contacts_exact_first_name(self):
        first_name = 'John'
        contacts = [
            Contact(id=1, first_name=first_name, last_name='Doe', email='email_1', user=self.user),
            Contact(id=2, first_name='Jane', last_name='Doe', email='email_2', user=self.user),
            Contact(id=3, first_name=first_name, last_name='Smith', email='email_3', user=self.user)
        ]
        filtered_contacts = [contact for contact in contacts if first_name.lower() in contact.first_name.lower()]
        mocked_contacts = MagicMock()
    ...
    async def test_search_contacts_exact_last_name(self):
        last_name = 'Doe'
        contacts = [
            Contact(id=1, first_name='John', last_name=last_name, email='email_1', user=self.user),
            Contact(id=2, first_name='Jane', last_name=last_name, email='email_2', user=self.user),
            Contact(id=3, first_name='John', last_name='Smith', email='email_3', user=self.user)
        ]
        filtered_contacts = [contact for contact in contacts if last_name.lower() in contact.last_name.lower()]
        mocked_contacts = MagicMock()
        mocked_contacts.scalars.return_value.all.return_value = filtered_contacts
        self.session.execute.return_value = mocked_contacts
    ...
    async def test_search_contacts_multiple_params(self):
        ...
        filtered_contacts = [contact for contact in contacts if first_name in contact.first_name]
        filtered_contacts = [contact for contact in filtered_contacts if last_name in contact.last_name]
        #filtered_contacts = [contact for contact in filtered_contacts if email in contact.email]
        mocked_contacts = MagicMock()
        mocked_contacts.scalars.return_value.all.return_value = filtered_contacts
        self.session.execute.return_value = mocked_contacts
        result = await search_contacts(self.session, self.user, first_name=first_name, last_name=last_name)
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0].first_name, first_name)
    ...
    async def test_get_upcoming_birthdays_one_contact(self):
        today = date.today()
        contact = Contact(id=1, first_name='John', last_name='Doe', birthday=today + timedelta(days=3), user=self.user)
        mocked_contacts = MagicMock()
        ...
        expected_result = [
            {
                ...
                "first_name": "John",
                "last_name": "Doe",
                "congratulation_date": date_to_string(adjust_for_weekend(contact.birthday.replace(year=today.year)))
            }
        ]
    ...
    async def test_get_upcoming_birthdays_multiple_contacts(self):
        today = date.today()
        contacts = [
            Contact(id=1, first_name='John', last_name='Doe', birthday=today + timedelta(days=1), user=self.user),
            Contact(id=2, first_name='Jane', last_name='Smith', birthday=today + timedelta(days=6), user=self.user),
            Contact(id=3, first_name='Mike', last_name='Johnson', birthday=today + timedelta(days=3), user=self.user)
        ]
        mocked_contacts = MagicMock()
        mocked_contacts.scalars.return_value.all.return_value = contacts
        self.session.execute.return_value = mocked_contacts
    ...
    def test_adjust_for_weekend(self):
        ...
        self.assertEqual(adjust_for_weekend(today - timedelta(days=1)), today - timedelta(days=1))
        self.assertEqual(adjust_for_weekend(today + timedelta(days=1)), today + timedelta(days=1))
