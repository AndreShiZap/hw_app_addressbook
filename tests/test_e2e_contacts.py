...
contact = {"first_name": "John", "last_name": "Snow", "email": "email", "phone": "123456789",
                "birthday": "2000-04-12", "data_add": ""}
...
def test_search_contacts_not_found(client, get_token, monkeypatch):
    with patch.object(auth_service, 'cache') as redis_mock:
        ...
        response = client.get("api/contact/search", headers=headers, params=search_params)
...
def test_upcoming_birthdays(client, get_token, monkeypatch):
    with (patch.object(auth_service, 'cache') as redis_mock):
        ...
        assert response.status_code == 200, response.text
        data = response.json()
        date_object = datetime.strptime(contact["birthday"], "%Y-%m-%d").date().replace(year=date.today().year)
