...
async def test_refresh_token_successful(client):
    ...
    async with TestingSessionLocal() as session:
        updated_user = await session.execute(select(User).where(User.email == test_user["email"]))
...
async def test_api_response_with_invalid_refresh_token(client):
    ...
    response = client.get("api/auth/refresh_token", headers={"Authorization": f"Bearer {invalid_refresh_token}"})
...
async def test_request_email_success(client):
    ...
    with patch("src.routes.auth.send_email", new_callable=AsyncMock) as mock_send_email:
        ...
        mock_send_email.assert_awaited_once_with(test_user["email"], test_user["username"], "http://testserver/")
        assert response.status_code == status.HTTP_200_OK
...
async def test_password_reset_request_user_not_found(client):
    async with TestingSessionLocal() as session:
        ...
        if current_user:
            current_user.confirmed = True
            await session.commit()
    response = client.post("api/auth/password-reset-request", params={"email": "no_correct@email.com"})
...
async def test_reset_password_success(client):
    ...
    with patch.object(auth_service, 'cache', new_callable=AsyncMock) as mock_redis:
        with patch("src.services.auth.auth_service.verify_email_token_from_redis") as mock_verify_token:
            ...
            assert response.json() == {"message": f"Password updated successfully for user {user_data['username']}"}
